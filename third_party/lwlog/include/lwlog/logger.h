...
namespace lwlog {
template <typename BufferLimits, typename LogExecutionPolicy,
          typename FlushPolicy, typename ThreadingPolicy,
          template <typename, typename, typename> typename... Sinks>
class logger {
  public:
    ...
    logger(std::string_view name, Iterator begin, Iterator end,
           SinkParams &&...params);
    template <typename... SinkParams>
    logger(std::string_view name, sink_list<BufferLimits> sink_list,
           SinkParams &&...params);
    template <typename... SinkParams>
    logger(std::string_view name, sink_ptr<BufferLimits> sink,
           SinkParams &&...params);
  ...
  private:
    template <typename... Args>
    void log(const details::log_message &log_msg, level log_level,
             Args &&...args);

  private:
    std::string_view m_name;
    typename LogExecutionPolicy::template backend<
        BufferLimits, typename ThreadingPolicy::concurrency_model_policy>
        m_backend;
};
} // namespace lwlog
