...
#include "lwlog/details/adaptive_waiter.h"
#include "lwlog/details/bounded_queue.h"
#include "lwlog/details/source_meta.h"
#include "lwlog/details/topic_registry.h"
#include "lwlog/level.h"
#include "lwlog/sinks/sink.h"

#include "lwlog/details/argument_formatter/argument_buffers_pool.h"
#include "lwlog/details/argument_formatter/argument_format.h"
...
struct synchronous_policy {
    ...
    template <typename BufferLimits, typename ConcurrencyModelPolicy,
              typename... Args>
    static void log(backend<BufferLimits, ConcurrencyModelPolicy> &backend,
                    const char *const message, level log_level,
                    const details::source_meta &meta, Args &&...args);
};

template <typename OverflowPolicy = block_overflow_policy,
...
struct asynchronous_policy {
    ...
    struct backend {
        ...
        struct queue_item;
        details::bounded_queue<Capacity, queue_item, OverflowPolicy,
  ...
  private:
    template <typename BufferLimits, typename ConcurrencyModelPolicy>
    static void
    process_item(backend<BufferLimits, ConcurrencyModelPolicy> &backend);
};
} // namespace lwlog
