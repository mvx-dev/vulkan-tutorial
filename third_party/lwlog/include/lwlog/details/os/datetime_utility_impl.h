...
timestamp_builder<Size> &timestamp_builder<Size>::append(std::size_t value) {
    ...
    const auto [ptr,
                ec]{std::to_chars(m_buffer + m_pos, m_buffer + Size, value)};
    m_pos = ptr - m_buffer;
...
template <std::size_t Size>
timestamp_builder<Size> &
timestamp_builder<Size>::append_ampm(std::size_t hour) {
...
template <std::size_t Size> const char *timestamp_builder<Size>::data() const {
    return m_buffer;
}
} // namespace lwlog::details::os::datetime
