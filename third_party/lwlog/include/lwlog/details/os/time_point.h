...
#include "lwlog/details/os/datetime_utility.h"
...
class time_point {
  public:
    time_point() {
        ...
        hour =
            datetime::handle_timezone(static_cast<std::uint8_t>(m_now.wHour));
        ...
        const std::uint64_t ticks_since_last_second{
            ticks_since_windows_epoch.QuadPart % 10'000'000};
        ...
        const std::time_t now_time_t{
            std::chrono::system_clock::to_time_t(m_now)};
        ...
        hour = datetime::handle_timezone(
            static_cast<std::uint8_t>(details.tm_hour));
